...
//
//  Covers are entered MANUALLY in the CMS:
//    1. Uploaded image (cover)      — admin uploaded one in the CMS
//    2. Manual URL (cover_url)      — admin pasted a cover link
//    3. "" (empty)                  — page shows a placeholder card
//
//  (Auto-fetching covers from Google Books / Open Library was removed —
//  covers are now supplied by hand, so no network lookups happen here.)
// ─────────────────────────────────────────────────────────────
...
async function main() {
  ...
  for (const file of files) {
    ...
    for (const b of fm.books || []) {
      // Covers are manual: uploaded image, else pasted URL, else blank
      // (the page renders a placeholder card when empty).
